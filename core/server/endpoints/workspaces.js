...
const { handleFileUpload, handlePfpUpload, handleProxyUpload } = require("../utils/files/multer");
...
function workspaceEndpoints(app) {
  ...
  app.post(
    ...
    async function (request, response) {
      console.log("[Workspace] Upload endpoint called");
      try {
        const { slug } = request.params; // Get slug from params

        const fs = require("fs");

        // Ensure request.file exists (handled by handleFileUpload middleware usually, but good to check)
        if (!request.file) {
          console.error("[Workspace] No file in request");
          response.status(400).json({ success: false, error: "No file uploaded" });
          return;
        }

        console.log("[Workspace] Processing file:", request.file.path);

        const { originalname, path: filePath, mimetype } = request.file;
        // const Collector = new CollectorApi(); // Initialize for logging (Removed to avoid legacy)

        // Create Native FormData specifically for Cortex
        const fileBuffer = fs.readFileSync(filePath);
        const blob = new Blob([fileBuffer], { type: mimetype });
        const formData = new FormData();
        formData.append("file", blob, originalname);
        formData.append("workspace_id", slug); // Use slug as workspace ID for Cortex
        formData.append("trigger_graph_indexing", "true");

        // Proxy to Cortex
        const cortexUrl =
          process.env.CORTEX_API_URL || "http://pandora-cortex:8000";
        console.log(`[Workspace] Proxying to Cortex: ${cortexUrl}`);

        const cortexRes = await fetch(`${cortexUrl}/api/v1/ingest/process`, {
          method: "POST",
          body: formData,
        });

        console.log("[Workspace] Cortex response status:", cortexRes.status);
        ...
        if (!cortexRes.ok) {
          ...
          console.error(`Cortex Error Body: ${errorText}`);
          throw new Error(
            `Cortex ingestion failed: ${cortexRes.status} ${cortexRes.statusText} - ${errorText}`
          );
        }

        const cortexData = await cortexRes.json();

        // Map Cortex response to frontend expectation
        const documents = [
          {
            location: `cortex://${cortexData.document_id}`,
            name: originalname,
            url: `cortex://${cortexData.document_id}`,
            title: originalname,
            docAuthor: "Unknown",
            description: "Imported via Cortex",
            docSource: "upload",
            chunkSource: originalname,
            published: new Date().toLocaleString(),
            wordCount: 0,
            token_count_estimate: 0,
            id: cortexData.document_id,
          },
        ];
  ...
  app.delete(
    ...
    async (request, response) => {
      try {
        ...
        const user = await userFromSession(request, response);
        const { ROLES } = require("../utils/middleware/multiUserProtected"); // Import locally or rely on top level

        // Protected System Workspaces (Knowledge Layers)
        const PROTECTED_LAYERS = ["system", "organization", "engineering"];
        if (PROTECTED_LAYERS.includes(slug)) {
          if (user?.role !== ROLES.admin) {
            console.warn(`[Workspace] User ${user?.username} attempted to delete protected layer ${slug}`);
            response.status(403).json({
              success: false,
              error: `Only Administrators can delete protected Knowledge Layer: ${slug}`
            });
            return;
          }
        }
        ...
        await WorkspaceSuggestedMessages.delete({ workspaceId: Number(workspace.id) });
        ...
        try {
          const { CortexClient } = require("../utils/cortexClient");
          await CortexClient.deleteWorkspace(slug);
        } catch (e) {
          console.error(`[Workspace] Failed to delete Cortex data for ${slug}:`, e.message);
        }
  ...
  app.post(
    ...
    async function (request, response) {
      console.log("[Workspace] Upload-and-embed endpoint called");
      try {
        const { slug = null } = request.params;
        const user = await userFromSession(request, response);
        const currWorkspace = multiUserMode(response)
        ...
        if (!cortexRes.ok) {
          throw new Error(
            `Cortex ingestion failed: ${cortexRes.status} ${cortexRes.statusText}`
          );
        }
        ...
        Collector.log(
          `Document ${originalname} proxied to Cortex (Job: ${cortexData.job_id})`
        );
        await Telemetry.sendTelemetry("document_uploaded_cortex");
        await EventLogs.logEvent(
          "document_uploaded",
          {
        ...
        // Map Cortex response to frontend expectation
        // Frontend expects { success: true, document: { id, location } }
        const document = {
          id: cortexData.document_id,
          location: `cortex://${cortexData.document_id}`,
        };

        // We might need to "fake" adding it to the local Postgres DB if the frontend expects it to be there instantly.
        // But for "Great Decoupling", we want Core to be stateless regarding vectors.
        // However, the `addDocuments` call in the original code updated the many-to-many relationship.
        // If we skip that, the document won't show up in the Workspace's document list in Core's UI
        // UNTIL we implement the "Fetch from Cortex" logic in getWorkspace.
        // For now, let's assume the Vector Inspector (Admin Console) is the primary goal, 
        // and the regular Workspace Chat UI might show it missing until Phase X.
        // But to pass "Success", we return what it wants.
