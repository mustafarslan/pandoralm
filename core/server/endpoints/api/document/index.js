...
function apiDocumentEndpoints(app) {
  ...
  app.post(
    ...
    */
      try {
        ...
        // Map workspace slug to ID (using the first one if multiple)
        const workspaceId = addToWorkspaces.split(",")[0] || "default";
        ...
        // Build headers - forward Authorization if present
        const headers = {};
        ...
        // Frontend expects { success: true, documents: [{...}] }
        const documents = [
          {
        ...
        // CRITICAL: Persist the record in Core's DB so it shows up in history
        if (!!addToWorkspaces) {
          await Document.api.uploadToWorkspace(
            addToWorkspaces,
            `cortex://${cortexData.document_id}`
          );
        }
  ...
  app.post(
    ...
    async (request, response) => {
      ...
      */
      try {
        const Collector = new CollectorApi(); // Keep for online check if needed, or remove
