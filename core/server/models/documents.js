...
const Document = {
  ...
  addDocuments: async function (workspace, additions = [], userId = null) {
    ...
    for (const path of additions) {
      // 1. Decoupling Check: If this is a Cortex-managed document, skip local embedding.
      if (path.startsWith("cortex://")) {
        const docId = path.split("://")[1];
        const newDoc = {
          docId,
          filename: "cortex-import.json", // Placeholder, Cortex owns the real filename
          docpath: path,
          workspaceId: workspace.id,
          metadata: JSON.stringify({
            title: "Processing in Background...",
            source: "cortex",
            description: "This document is being processed by the Pandora Cortex engine.",
        ...
        try {
          ...
          embedded.push(path);
          console.log(`[DECOUPLED] Document ${docId} delegated to Cortex. Skipped local vectorization.`);
        } catch (error) {
          console.error(`[DECOUPLED] Failed to register Cortex doc ${docId}:`, error.message);
