...
/**
 * Handle Proxy file upload - saves to temp dir instead of hotdir
 */
const proxyUploadStorage = multer.diskStorage({
  destination: function (_, __, cb) {
    const uploadOutput = os.tmpdir();
    cb(null, uploadOutput);
  },
  filename: function (_, file, cb) {
    // Keep original name for Cortex to process
    console.log("[Multer] Processing filename:", file.originalname);
    file.originalname = normalizePath(
      Buffer.from(file.originalname, "latin1").toString("utf8")
    );
    cb(null, file.originalname);
...
/**
 * Handle Proxy file upload for Cortex ingestion
 */
function handleProxyUpload(request, response, next) {
  console.log("[Multer] handleProxyUpload called");
  const upload = multer({ storage: proxyUploadStorage }).single("file");
  upload(request, response, function (err) {
    if (err) {
    ...
    console.log("[Multer] File uploaded to temp:", request.file?.path);
    next();
  });
}
