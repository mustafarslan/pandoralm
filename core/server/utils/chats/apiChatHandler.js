...
async function chatSync({
  ...
  // Use Cortex for vector operations instead of local VectorDb
  // This ensures consistent embedding models (Cortex uses OpenAI text-embedding-3-small)
  const messageLimit = workspace?.openAiHistory || 20;
  const hasVectorizedSpace = await CortexClient.hasVectors(workspace.slug);
  const embeddingsCount = await CortexClient.getVectorCount(workspace.slug);
  ...
  const vectorSearchResults =
    embeddingsCount !== 0
      ? await CortexClient.performSimilaritySearch({
        ...
        // TODO: Pass workspace.allowedLayers when knowledge layer mapping is implemented
        allowedLayers: [],
      })
      : {
        contextTexts: [],
