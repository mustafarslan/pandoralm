...
async function streamChatWithForEmbed(
  ...
  // Use Cortex for vector operations
  const messageLimit = embed.message_limit ?? 20;
  const hasVectorizedSpace = await CortexClient.hasVectors(embed.workspace.slug);
  const embeddingsCount = await CortexClient.getVectorCount(embed.workspace.slug);
  ...
  const vectorSearchResults =
    embeddingsCount !== 0
      ? await CortexClient.performSimilaritySearch({
        query: message,
        workspaceId: embed.workspace.slug,
        topK: embed.workspace?.topN || 4,
