...
const LanceDb = {
  ...
  /**
   * Gets the LanceDB table name for a namespace (workspace slug).
   * This MUST match the naming convention used by Cortex's vector_store.py:
   *   table_name = f"{prefix}_{safe_id}"
   * where prefix="vectors" and safe_id has hyphens replaced with underscores.
   * @param {string} namespace - The workspace slug or identifier
   * @returns {string} The LanceDB table name
   */
  _getTableName: function (namespace) {
    if (!namespace) return null;
    const safeId = String(namespace).replace(/-/g, "_");
    return `vectors_${safeId}`;
  },
  ...
  namespaceCount: async function (_namespace = null) {
    const { client } = await this.connect();
    const tableName = this._getTableName(_namespace);
  ...
  rerankedSimilarityResponse: async function ({
    ...
    const tableName = this._getTableName(namespace);
    const collection = await client.openTable(tableName);
  ...
  namespace: async function (client, namespace = null) {
    ...
    const tableName = this._getTableName(namespace);
    const collection = await client.openTable(tableName).catch(() => false);
  ...
  updateOrCreateCollection: async function (client, data = [], namespace) {
    ...
    await client.createTable(tableName, data);
    return true;
  },
  ...
  namespaceExists: async function (client, namespace = null) {
    ...
    return collections.includes(tableName);
  },
  /**
   *
   * @param {LanceClient} client
