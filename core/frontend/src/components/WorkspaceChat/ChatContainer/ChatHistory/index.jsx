...
export default function ChatHistory({
  ...
  if (history.length === 0 && !hasAttachments) {
    return (
      <div className="flex flex-col flex-1 h-full md:mt-0 pb-44 md:pb-40 w-full justify-end items-center">
        <div className="flex flex-col items-center md:items-start md:max-w-[600px] w-full px-4">
          <p className="text-[#252525] text-lg font-semibold py-4">
            {t("chat_window.welcome")}
          </p>
          {!user || user.role !== "default" ? (
            <p className="w-full items-center text-[#545454] text-lg font-base flex flex-col md:flex-row gap-x-1">
              {t("chat_window.get_started")}
              <span
                className="underline font-semibold cursor-pointer text-[#252525] hover:text-[#7D7D7D]"
                onClick={showModal}
              >
                {t("chat_window.upload")}
              </span>
              {t("chat_window.or")}{" "}
              <b className="font-semibold text-[#252525]">{t("chat_window.send_chat")}</b>
            </p>
          ) : (
            <p className="w-full items-center text-[#545454] text-lg font-base flex flex-col md:flex-row gap-x-1">
