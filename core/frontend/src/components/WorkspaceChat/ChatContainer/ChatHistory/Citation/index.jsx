...
export default function Citations({ sources = [] }) {
  ...
  return (
    <div className="flex flex-col mt-4 pt-4 border-t border-[#252525]/10 justify-left">
      <button
        onClick={() => setOpen(!open)}
        className={`border-none font-semibold text-gray-500 font-medium italic ${textSizeClass} text-left ml-14 pt-2 ${open ? "pb-2" : ""
          } hover:text-[#252525] transition-all duration-300`}
      >
        {open
        ...
        <CaretRight
          weight="bold"
          size={14}
          className={`inline-block ml-1 transform transition-transform duration-300 ${open ? "rotate-90" : ""
            }`}
        />
      </button>
      {open && (
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-start gap-y-[0.2px] px-1">
        <p
          className={`!m-0 font-semibold whitespace-nowrap text-[#252525] hover:opacity-80 ${textSizeClass}`}
        >
          {truncatedTitle}
        </p>
        <p
          className={`!m-0 text-[10px] font-medium text-gray-500 ${textSizeClass}`}
