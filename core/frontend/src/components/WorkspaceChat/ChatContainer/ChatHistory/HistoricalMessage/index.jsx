...
const HistoricalMessage = ({
  ...
  if (!!error) {
    return (
      <div
        key={uuid}
        className={`flex justify-start items-end w-full bg-theme-bg-chat`}
      >
  ...
  return (
    <div
      ...
      className={`${isDeleted ? "animate-remove" : ""
        } flex justify-start items-end w-full group`}
    >
      <div className="py-8 px-4 w-full flex gap-x-5 md:max-w-[80%] flex-col">
        <div className={`flex gap-x-5 ${alignmentCls}`}>
          ...
          {isEditing ? (
            ...
            <div
              className={`break-words ${role === "user"
                ? "bg-[#252525] text-white p-4 rounded-2xl rounded-tl-sm shadow-sm leading-relaxed max-w-[80%] float-right"
                : "bg-transparent text-[var(--ink-body)] p-0 pl-0"
                }`}
            >
              <RenderChatContent
                role={role}
              ...
              {isRefusalMessage && (
                ...
                >
                  <div className="flex flex-row items-center gap-x-1 group-hover:opacity-100 opacity-60 w-fit">
                    ...
                    <p className="!m-0 !p-0 text-gray-500 !no-underline text-xs cursor-pointer">
...
function ProfileImage({ role, workspace }) {
  if (role === "assistant" && workspace.pfpUrl) {
    return (
      <div className="relative w-[35px] h-[35px] rounded-full flex-shrink-0 overflow-hidden shadow-[0_0_10px_rgba(37,37,37,0.15)]">
        <img
          src={workspace.pfpUrl}
          alt="Workspace profile picture"
          className="absolute top-0 left-0 w-full h-full object-cover rounded-full bg-os-surface"
        />
      </div>
    );
  }
