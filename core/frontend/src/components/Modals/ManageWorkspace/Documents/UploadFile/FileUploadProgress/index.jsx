...
function FileUploadProgressComponent({
  ...
  return (
    <div
      className={`${isFadingOut ? "file-upload-fadeout" : "file-upload"
        } h-14 px-2 py-2 flex items-center gap-x-4 rounded-lg bg-[#FFFFFF] border border-[#545454] shadow-md`}
    >
      <div className="w-6 h-6 flex-shrink-0">
        {status !== "complete" ? (
      ...
      <div className="flex flex-col">
        <p className="text-[#252525] text-xs font-medium">
          {truncate(file.name, 30)}
        </p>
        <p className="text-[#7D7D7D] text-xs font-medium">
