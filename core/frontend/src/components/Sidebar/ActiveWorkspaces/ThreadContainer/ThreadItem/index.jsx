...
export default function ThreadItem({
  ...
  return (
    ...
    >
      ...
      <div
        className={`${isActive
          ? "border-l-2 border-b-2 border-[#7D7D7D] z-[2]"
          : "border-l-2 border-b-2 border-[#7D7D7D] z-[1]"
          } h-1/2 absolute top-0 left-[18px] w-3 rounded-bl-sm`}
      ></div>
      {/* Downstroke border for next item - extends beyond this item */}
      {hasNext && (
        <div
          className={`${idx <= activeIdx && !isActive
            ? "border-l-2 border-[#7D7D7D] z-[2]"
            : "border-l-2 border-[#7D7D7D] z-[1]"
            } h-[calc(100%+12px)] absolute top-0 left-[18px] w-3`}
        ></div>
      )}
      ...
      />
      <div
        className={`flex w-full items-center justify-between pr-2 group relative pl-9 ${isActive ? "bg-white border-2 border-[#252525] text-[#252525] font-semibold" : "bg-white border border-[#CFCFCF] text-[#545454] hover:border-[#7D7D7D] hover:text-[#252525]"} rounded-md py-2`}
      >
        {thread.deleted ? (
          ...
          >
            <p
              className={`text-left text-sm truncate max-w-[150px] ${isActive ? "font-medium text-black" : "text-[#545454] group-hover:text-[#252525]"
                }`}
            >
              {thread.name}
            </p>
