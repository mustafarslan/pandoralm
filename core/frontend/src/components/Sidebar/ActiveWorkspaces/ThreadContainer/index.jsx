...
export default function ThreadContainer({ workspace }) {
  ...
  return (
    <div className="flex flex-col gap-y-3 mt-2 ml-4" role="list" aria-label="Threads">
      <ThreadItem
        idx={0}
...
function NewThreadButton({ workspace }) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full relative flex h-[40px] items-center border border-[#CFCFCF] hover:bg-[#EBEBEB] hover:border-[#A0A0A0] rounded-lg transition-all"
    >
      <div className="flex w-full gap-x-2 items-center pl-4">
        <div className="bg-[#E8E8E8] p-2 rounded-lg h-[24px] w-[24px] flex items-center justify-center">
          {loading ? (
            <CircleNotch
              weight="bold"
              size={14}
              className="shrink-0 animate-spin text-[#545454]"
            />
          ) : (
            <Plus
              weight="bold"
              size={14}
              className="shrink-0 text-[#545454]"
            />
          )}
        </div>

        {loading ? (
          <p className="text-left text-[#545454] text-sm font-medium">
            Starting Thread...
          </p>
        ) : (
          <p className="text-left text-[#545454] text-sm font-medium">
