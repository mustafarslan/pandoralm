...
export default {
  ...
  github: () => {
    return "https://github.com/mustafarslan/pandoralm";
  },
  // Deprecated: Discord and docs links removed for PandoraLM branding
  discord: () => {
  ...
  workspace: {
    ...
    settings: {
      ...
      knowledgeLayers: (slug) => {
        return `/workspace/${slug}/settings/knowledge-layers`;
      },
    },
  ...
  settings: {
    ...
    // Keep alias for backward compatibility/hidden callers
    appearance: () => {
      return "/settings/interface";
    },
    branding: () => {
    ...
    // Admin Console (Vector/Graph Operations)
    adminConsole: {
      overview: () => `/settings/admin-console`,
      vectors: () => `/settings/admin-console/vectors`,
      entities: () => `/settings/admin-console/entities`,
      relationships: () => `/settings/admin-console/relationships`,
      communities: () => `/settings/admin-console/communities`,
    },
  },
  agents: {
    builder: () => {
  ...
  communityHub: {
    website: () => {
      return import.meta.env.DEV
        ? `http://localhost:5173`
        : `https://pandoralm.dev/hub`; // Updated for PandoraLM
    },
    /**
