...
import KnowledgeLayersSettings from "./KnowledgeLayers";
...
const TABS = {
  ...
  "knowledge-layers": KnowledgeLayersSettings,
};
  ...
  return (
    <div className="w-screen h-screen overflow-hidden bg-theme-bg-container flex">
      ...
      <div
        ...
        className="transition-all duration-500 relative md:ml-[2px] md:mr-[16px] md:my-[16px] md:rounded-[16px] bg-os-bg w-full h-full overflow-y-scroll"
      >
        <div className="flex gap-x-10 pt-6 pb-4 ml-16 mr-8 border-b border-[#CFCFCF]">
          <Link
            to={paths.workspace.chat(slug)}
            className="absolute top-2 left-2 md:top-4 md:left-4 transition-all duration-300 p-2 rounded-full text-[#545454] bg-white border border-[#CFCFCF] hover:bg-[#F2F2F2] hover:text-[#252525] z-10"
          >
          ...
          />
          <TabItem
            title={t("workspaces—settings.knowledge_layers") || "Knowledge Layers"}
            icon={<Stack className="h-6 w-6" />}
            to={paths.workspace.settings.knowledgeLayers(slug)}
          />
        </div>
...
function TabItem({ title, icon, to, visible = true }) {
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        `${isActive
          ? "text-[#252525] pb-4 border-b-[4px] -mb-[19px] border-sky-400"
