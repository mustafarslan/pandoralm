...
const Workspace = {
  ...
  streamChat: async function ({ slug }, message, handleChat, attachments = [], mode = "auto") {
  ...
  uploadFile: async function (slug, formData, layerId = "user") {
    ...
    if (layerId) {
      headers["X-Pandora-Layer-ID"] = layerId;
    }
    delete headers["Content-Type"]; // Allow browser to set boundary for FormData
