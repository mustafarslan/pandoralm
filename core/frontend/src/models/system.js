...
const System = {
  cacheKeys: {
    footerIcons: "pandoralm_footer_links",
    supportEmail: "pandoralm_support_email",
    customAppName: "pandoralm_custom_app_name",
    canViewChatHistory: "pandoralm_can_view_chat_history",
    deploymentVersion: "pandoralm_deployment_version",
  },
  ping: async function () {
  ...
  getWorkerStats: async function () {
    return await fetch(`${API_BASE}/ops/workers`, {
      headers: baseHeaders(),
    })
      .then((res) => res.json())
      .catch((e) => {
  ...
  listActiveJobs: async function () {
    return await fetch(`${API_BASE}/ops/jobs/active`, {
      headers: baseHeaders(),
    })
      .then((res) => res.json())
      .catch((e) => {
  ...
  revokeJob: async function (jobId) {
    return await fetch(`${API_BASE}/ops/jobs/${jobId}/revoke`, {
      method: "POST",
      headers: baseHeaders(),
    })
      .then((res) => res.json())
  ...
  switchTable: async function (tableName) {
    return await fetch(`${API_BASE}/v1/ops/switch-table`, {
      method: "POST",
      headers: baseHeaders(),
      body: JSON.stringify({ table_name: tableName }),
    })
      .then((res) => {
        if (!res.ok) throw new Error("Could not switch table.");
        return res.json();
      })
      .catch((e) => {
        console.error(e);
  ...
  getWorkerStats: async function () {
    return await fetch(`${API_BASE}/v1/ops/health/overview`, {
      method: "GET",
      headers: baseHeaders(),
    })
      .then((res) => {
        if (!res.ok) throw new Error("Could not fetch worker stats.");
        return res.json();
      })
      .catch((e) => {
        console.error(e);
  ...
  listActiveJobs: async function () {
    try {
      // Fetch both active (Celery RAM) and history (Postgres)
      const [activeRes, historyRes] = await Promise.all([
        fetch(`${API_BASE}/v1/ops/jobs/active`, { headers: baseHeaders() }).then(r => r.json()).catch(() => ({ jobs: [] })),
        fetch(`${API_BASE}/v1/ops/history/jobs`, { headers: baseHeaders() }).then(r => r.json()).catch(() => ({ jobs: [] }))
      ]);

      const activeJobs = activeRes.jobs || [];
      const historyJobs = historyRes.jobs || [];
  ...
  rebuildGraph: async function () {
    return await fetch(`${API_BASE}/v1/ops/graph/rebuild-all`, {
      method: "POST",
      headers: baseHeaders(),
    })
      .then((res) => {
        if (!res.ok) throw new Error("Could not trigger graph rebuild.");
        return res.json();
      })
      .catch((e) => {
        console.error(e);
  ...
  listLayers: async function (includeCounts = false) {
    // UPDATED: Point to Cortex Ops API instead of missing admin endpoint
    return await fetch(`${API_BASE}/v1/ops/layers?include_vector_counts=${includeCounts}`, {
      method: "GET",
      headers: baseHeaders(),
    })
      .then((res) => {
        if (!res.ok) throw new Error("Could not fetch layers.");
        return res.json(); // Expects list of { id, name, type, ... }
      })
      .catch((e) => {
        console.error(e);
        return [];
  ...
  /**
   * This endpoint resolves Keycloak roles to Layer access permissions.
   * @returns { Promise< { layers: Array < { id: string, name: string, type: string, color: string, access: string, permissions: string[] } >, error: string | null } >}
   */
  fetchAuthLayers: async function () {
    return fetch(`${API_BASE}/system/auth-layers`, {
      method: "GET",
      headers: baseHeaders(),
    })
      .then((res) => {
        if (!res.ok) throw new Error("Could not fetch auth layers.");
        return res.json();
      })
      .then((res) => ({ layers: res.layers || [], error: null }))
      .catch((e) => {
        console.error("[System.fetchAuthLayers]", e);
        return { layers: [], error: e.message };
      });
  },

  getMyLayers: async function () {
    return await fetch(`${API_BASE}/v1/layers`, {
      method: "GET",
      headers: baseHeaders(),
    })
      .then((res) => {
        if (!res.ok) throw new Error("Could not fetch my layers.");
        return res.json();
      })
      .catch((e) => {
        console.error(e);
  ...
  getMyPrivateLayer: async function () {
    return await fetch(`${API_BASE}/v1/layers/me`, {
      method: "GET",
      headers: baseHeaders(),
    })
      .then((res) => {
        if (!res.ok) throw new Error("Could not fetch private layer.");
        return res.json();
      })
      .catch((e) => {
        console.error(e);
  ...
  fetchPendingGoldSamples: async function () {
    return fetch(`${API_BASE}/evaluation/gold-standard/pending`, {
      method: "GET",
      headers: baseHeaders(),
    })
      .then((res) => {
  ...
  verifyGoldSample: async function (sampleId, action) {
    return fetch(
      `${API_BASE}/evaluation/gold-standard/${sampleId}/verify?action=${action}`,
      {
        method: "POST",
        headers: baseHeaders(),
      }
      ...
      .then((res) => {
        if (!res.ok) throw new Error("Failed to verify sample.");
        return res.json();
      })
      .catch((e) => {
        console.error(e);
  ...
  updateLayerQuota: async function (layerId, data) {
    return await fetch(`${API_BASE}/v1/admin/layers/${layerId}/quota`, {
      method: "PUT",
      headers: baseHeaders(),
      body: JSON.stringify(data),
    })
      .then((res) => {
        if (!res.ok) throw new Error("Failed to update layer quota.");
        return res.json();
      })
      .catch((e) => {
        console.error(e);
  ...
  bulkUpdateQuotas: async function (data) {
    return await fetch(`${API_BASE}/v1/admin/layers/quotas/bulk`, {
      method: "PUT",
      headers: baseHeaders(),
      body: JSON.stringify(data),
    })
      .then((res) => {
        if (!res.ok) throw new Error("Failed to bulk update quotas.");
        return res.json();
      })
      .catch((e) => {
        console.error(e);
  ...
  getQualityGateStatus: async function () {
    return await fetch(`${API_BASE}/v1/evaluation/latest`, {
      method: "GET",
      headers: baseHeaders(),
    })
      .then((res) => {
        if (!res.ok) throw new Error("Could not fetch quality gate status.");
        return res.json();
      })
      .catch((e) => {
        console.error(e);
        // Fallback Mock for UI Development/Demo
        return {
          status: 'PASS',
